...
ВАЖНО: ИСПОЛЬЗУЙ ИНСТРУМЕНТЫ (write_file, edit_file, execute_command и т.д.) ля реализации этого шага! Не возвращай JSON структуру с описанием изменений. Твоя задача - физически внести изменения в файлы проекта (или создать новые) с полным, рабочим кодом.

1. Если action_type = create_file, используй инструмент write_file для создания нового файла с полным кодом (не оставляй комментарии вроде "тут будет код").
2. Если action_type = modify_file, сначала используй read_file для чтения, затем edit_file или multi_edit_file для внесения точечных изменений. ВАЖНО: Если файл для модификации не существует (ты его не нашел), ты ОБЯЗАН создать его с нуля с помощью write_file! Не сдавайся и не пиши просто отчет, код должен быть написан!
3. Если action_type = delete_file, используй delete_file.
4. Если action_type = run_command, используй execute_command. ОБЯЗАТЕЛЬНО проверь результат команды. Если код для тестирования отсутствует - создай его перед тестированием! (testing without code is stupid).
5. Если action_type = verify, выполни нужные проверки, тесты.

ЗАПРЕТ: Не используй search_files бесконечно. Если ты не нашел нужный контракт или код, значит проект создается с нуля и тебе нужно написать этот код самому через write_file в правильную папку (например, `src/` или `contracts/`).

Когда ты закончишь выполнение этого шага и убедишься, что все действия (через инструменты) успешно вносят изменения на диск, верни краткий текстовый отчет о том, что было сделано (что за файлы созданы, какие тесты запущены).
"""
